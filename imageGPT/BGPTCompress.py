...
# Add parent directory to path to import from RAGLMCompress
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'RAGLMCompress'))
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('compression.log', mode='w')
    ]
)
...
def bgpt_compress(compress_input, logits, metric, precision=None, prefix_length=None):
    ...
    """
    logger.debug("Starting bGPT compression...")
    ...
    logger.debug(f"Compression parameters: precision={precision}, prefix_length={prefix_length}")
    logger.debug(f"Input shape: {compress_input.shape}, Logits shape: {logits.shape}")
        
    output = []
    # Initialize a Encoder Object
    logger.debug("Initializing arithmetic encoder...")
    ...
    logger.debug(f"Encoding sequence length: {target_sequence_to_encode.shape[1]}")
    logger.debug("Computing probabilities from logits...")
    ...
    logger.debug(f"Encoding {len(sequence_array)} symbols with arithmetic coding...")
    encode_start = time.time()
    for i, (symbol, prob, pd_prob) in enumerate(zip(sequence_array, probs, pd)):
        if (i + 1) % 1000 == 0:
            logger.debug(f"  Encoded {i+1}/{len(sequence_array)} symbols...")
        encoder.encode(
            ac_utils.normalize_pdf_for_arithmetic_coding(prob, np.float32), symbol
        )
    encoder.terminate()
    encode_time = time.time() - encode_start
    logger.debug(f"Arithmetic encoding completed in {encode_time:.2f} seconds")

    # to visualize and compute metrics, map to str
    logger.debug("Converting compressed bits to bytes...")
    ...
    logger.info(f"  Compressed: {metric.compressed_length} bytes")
    logger.info(f"  Original: {metric.total_length} bytes")
    logger.info(f"  Compression ratio: {compress_ratio:.6f} ({compress_ratio*100:.2f}% of original)")
    logger.info(f"  Compression rate: {compress_rate:.6f}x")
    logger.debug(f"  Padded bits: {num_padded_bits}")
...
def bgpt_decode(
    ...
    """
    logger.debug(f"Starting bGPT decode: seq_len={original_seq_len}, compressed_bytes={len(compressed_bytes)}")
    
    if precision is None:
        precision = CompressionConfig.PRECISION
        
    # convert bytes back to bit stream
    logger.debug("Converting compressed bytes to bit stream...")
    data_iter = iter(
    ...
    logger.debug(f"Initializing arithmetic decoder (precision={precision})...")
    ...
    logger.debug(f"Decoding {original_seq_len} symbols...")
    decode_start = time.time()
    for i in range(original_seq_len):
        if (i + 1) % 100 == 0:
            logger.debug(f"  Decoded {i+1}/{original_seq_len} symbols...")
        ...
        if do_test:
            ...
            logger.debug(f"target diff: {target_diff}")
            if original_sequence[i] != de_token:
                logger.warning(f"Mismatch at position {i}: expected {original_sequence[i]}, got {de_token}")
                import pdb
                pdb.set_trace()
    
    decode_time = time.time() - decode_start
    logger.debug(f"Decoding completed in {decode_time:.2f} seconds")
...
def load_bgpt_model(checkpoint_path, device):
    ...
    """
    logger.info("=" * 80)
    logger.info("Loading bGPT model...")
    ...
    logger.debug(f"Creating patch config: {CompressionConfig.PATCH_NUM_LAYERS} layers, "
                f"hidden_size={CompressionConfig.HIDDEN_SIZE}, "
                f"patch_length={CompressionConfig.PATCH_LENGTH}")
    ...
    logger.debug(f"Creating byte config: {CompressionConfig.BYTE_NUM_LAYERS} layers, "
                f"patch_size={CompressionConfig.PATCH_SIZE}")
    ...
    logger.debug("Initializing bGPTLMHeadModel...")
    llm = bGPTLMHeadModel(patch_config, byte_config)

    logger.info(f"Loading checkpoint from {checkpoint_path}...")
    checkpoint_start = time.time()
    # Load checkpoint with CPU mapping (checkpoint may have been saved on GPU)
    checkpoint = torch.load(checkpoint_path, map_location=device)
    checkpoint_load_time = time.time() - checkpoint_start
    logger.info(f"Checkpoint loaded in {checkpoint_load_time:.2f} seconds")
    ...
    logger.debug(f"State dict loaded in {state_dict_time:.2f} seconds")
...
def test_bmp_compression(
    ...
    logger.info(f"Found {len(bmp_files)} BMP files to test")
    ...
    for file_idx, bmp_file in enumerate(bmp_files, 1):
        logger.info("")
        logger.info("=" * 80)
        logger.info(f"Processing file {file_idx}/{len(bmp_files)}: {os.path.basename(bmp_file)}")
        logger.info("=" * 80)
        ...
        original_file_size = os.path.getsize(bmp_file)
        logger.info(f"Original file size: {original_file_size:,} bytes ({original_file_size/1024:.2f} KB)")
        
        # Step 1: Split BMP into patches
        logger.info("")
        logger.info("-" * 80)
        logger.info(f"Step 1: Splitting BMP into {patch_size}x{patch_size} patches...")
        ...
        logger.info(f"✓ Created {len(patch_files)} patches in {split_time:.2f} seconds")
        logger.debug(f"Patches saved to: {patches_subfolder}")
        
        # Step 2: Compress each patch
        logger.info("")
        logger.info("-" * 80)
        logger.info(f"Step 2: Compressing {len(patch_files)} patches...")
        compression_start = time.time()
        compressed_info = {}  # Store compression info for each patch
        
        for patch_idx, patch_file in enumerate(tqdm(patch_files, desc="Compressing patches", unit="patch"), 1):
            patch_name = os.path.basename(patch_file)
            patch_id = os.path.splitext(patch_name)[0]
            
            logger.debug(f"  Processing patch {patch_idx}/{len(patch_files)}: {patch_id}")
            
            # Read patch bytes
            read_start = time.time()
            bytes_list, ext = read_bytes(patch_file)
            read_time = time.time() - read_start
            logger.debug(f"    Read {len(bytes_list)} bytes in {read_time:.3f}s")
            
            # Prepare input
            pad_start = time.time()
            padded_segment = pad_input_for_bgpt([bytes_list], [ext], device)
            pad_time = time.time() - pad_start
            logger.debug(f"    Padded input in {pad_time:.3f}s")
            ...
            with torch.inference_mode():
                ...
                logger.debug(f"    Running model inference (input shape: {input_ids.shape})...")
                output = model(patches=input_ids, masks=attention_mask)
                logits = output.logits
            ...
            logger.debug(f"    Model inference completed in {inference_time:.3f}s")
            ...
            logger.debug(f"    Compression completed in {compress_time:.3f}s")
            ...
            logger.debug(f"    Saved compressed data to {compressed_path}")
        ...
        compression_total_time = time.time() - compression_start
        compress_rate, compress_ratio = total_metric.compute_ratio()
        logger.info(f"✓ Compression completed in {compression_total_time:.2f} seconds")
        logger.info(f"  Overall compression ratio: {compress_ratio:.6f} ({compress_ratio*100:.2f}% of original)")
        ...
        logger.info("")
        logger.info(f"Total processing time for {filename}: {file_total_time:.2f} seconds")
        logger.info("=" * 80)
    
    logger.info("")
    logger.info("=" * 80)
    logger.info("BMP COMPRESSION TEST COMPLETED!")
    logger.info("=" * 80)
    logger.info(f"Total files processed: {len(bmp_files)}")
    logger.info(f"Reconstructed images saved to: {output_folder}")
